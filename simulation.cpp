...
#include <chrono>
#include <fstream>   //file read/write ops
#include <algorithm> //used default sorting ops
...
const double BASE_WORK_INCOME = 10000;         // Base income from working
const double WORK_POLLUTION_IMPACT = 1.5;      // Pollution generated by working
...
// New function to extract tips from educationalTips.txt (for scaling and better code practice)
...
// built mergesort for template use case and function overloading
// used template to cater to different vector types
template <typename T>
void merge(vector<T> &vec, int l, int m, int r)
{

    vector<T> leftV(vec.begin() + l, vec.begin() + m + 1);
    vector<T> rightV(vec.begin() + m + 1, vec.begin() + r + 1);
...
void mergeSort(vector<T> &vec, int l, int r)
{
    if (l < r)
    {
        ...
        mergeSort(vec, l, mid);     // sort left
        mergeSort(vec, mid + 1, r); // sort right
        merge(vec, l, mid, r);      // merge left and right
    }
}
...
{
protected:
    double capacity, currentUsage; // set protected to enable inheritance with Water, Gas and Electricity Derived classes

public:
    Utilities(double cap = 0) : capacity(cap), currentUsage(0) {}
    virtual ~Utilities() {} // allow for derived class destructor to run
...
// Derived classes of Base class utility
...
public:
    ...
    // ~Water() { cout << "Water infrastructure is destroyed.\n"; }
};
...
public:
    ...
    // ~Electricity() { cout << "Electricity infrastructure is destroyed.\n"; }
};
...
public:
    ...
    // ~Gas() { cout << "Gas infrastructure is destroyed.\n"; }
};
...
public:
        ...
        : Building("Power Plant", land, cost), energyGencapacity(capacity), energyType(eType)
    {
        if (energyType == "Coal")
        {
            pollutionGenerated = 40;
            cost = 80000;
        }
        else if (energyType == "Gas")
        {
            pollutionGenerated = 25;
            cost = 100000;
        }
        else if (energyType == "Solar")
        {
            pollutionGenerated = 0.5;
            cost = 200000;
            capacity = 8000;
        }
        else if (energyType == "Wind")
        {
            pollutionGenerated = 0.8;
            cost = 250000;
            capacity = 7500;
        }
...
// Derived classes of base class transport
...
protected:
    string type;
    double cost, environmentalImpact;
...
public:
    ...
    // allow for modularity. Better code practice
    ...
    void checkGreenLevelUp(double ecoScore)
    {
        ...
        if (newLevel > oldLevel)
        {
            ...
            addReward("Nature's Ally, for reaching Green Level " + to_string(greenLevel), greenLevel * 15000, greenLevel * 75); // Increased rewards
        }
    ...
    // Ensuring no repeated awrds and checking of awards is done later
    void addReward(string description, double goldAmount, double xpAmount)
    {
        cout << "\n*** Title Earned: " << description << " ***" << endl;
        if (goldAmount > 0)
            earnGold(goldAmount);
        if (xpAmount > 0)
...
class Environment
{
    ...
    // no derived class, so no need of protected attributes
    // used friend functions and class members funcs to access data
...
public:
    ...
    // calculates population checking residential building types
    ...
    // improve greenspace (eg on planting tree)
    void improveGreenSpace(double amount)
    {
        greenSpaceFactor += amount;
        if (greenSpaceFactor < 0.1)
            greenSpaceFactor = 0.1;
        recalculateEcoScore();
    }
    ...
    // improve transport network score
    void updateTransportScore(const vector<Transport *> &transport)
    {
        ...
        for (const auto *t : transport)
        {
            ...
            if (dynamic_cast<const Road *>(t))
                transportNetworkScore += 5 * maintenanceFactor;
            else if (dynamic_cast<const Railways *>(t))
                transportNetworkScore += 20 * maintenanceFactor;
            else if (dynamic_cast<const Airport *>(t))
                transportNetworkScore += 40 * maintenanceFactor;
        }
    }
    ...
    {
        if (totalPowerCapacity <= 0)
            return 0.0;
        return (totalRenewablePowerCapacity / totalPowerCapacity) * 100.0;
    }

    // operator overloading << to display Environment
...
// Used Template for generalisation of data being logged
template <typename T>
class CityLog
{
    ofstream logFile;
...
class City
{
    ...
    // protected attributes were not needed (no inheritance)
    string cityName;
    Player cityPlayer;
    ...
    // return true if a goal is alr achieved
    // avoid repeated titlles
...
public:
    // constructor
    City(string name, string playerName) : cityName(name),
    ...
    ~City()
    {
        for (Building *b : buildings)
            delete b;
        for (Transport *t : transportSystems)
            delete t;
    ...
    // old function, replaced with template for better code practice
    ...
    void GoToMall()
    {
        ...
        if (!mallExists)
        {
            cout << "There's no mall in the city!" << endl;
            return;
        }
        double cost = 75.0;
        if (cityPlayer.spendGold(cost))
    ...
    // operator overloading <<. Display city
    friend ostream &operator<<(ostream &os, const City &city)
    {
        ...
        if (city.transportSystems.empty())
            os << "No transport systems yet." << endl;
        else
        {
    ...
    void Compete(const City &opponentCity) const
    {
        ...
        if (myScore > opponentScore)
            cout << cityName << " has a higher EcoScore!" << endl;
        else if (opponentScore > myScore)
            cout << opponentCity.cityName << " has a higher EcoScore!" << endl;
        else
            cout << "Both cities have the same EcoScore!" << endl;

        cout << "----------------------------------------------------\n"
             << endl;
    }

    // check goals and give titles
    void checkGoalsAndGrantRewards()
    {
        ...
        {
            cityPlayer.addReward(goalRenewable + ", for achieving >=75% Renewable Power.", 75000, 600);
            achievedGoals.push_back(goalRenewable);
        }
        string goalLowPollution = "Clear Skies Champion";
        if (!hasAchievedGoal(goalLowPollution) && currentPollution <= 12.0)
        {
            cityPlayer.addReward(goalLowPollution + ", for having City Pollution <= 12.", 30000, 400);
            achievedGoals.push_back(goalLowPollution);
        }
        string goalTopEco = "Green City Visionary";
        if (!hasAchievedGoal(goalTopEco) && currentEcoScore >= 95.0)
        {
            cityPlayer.addReward(goalTopEco + ", for achieving EcoScore >= 95.", 150000, 1200);
            achievedGoals.push_back(goalTopEco);
        }
        string goalFirstEV = "Watt a Ride!";
        if (!hasAchievedGoal(goalFirstEV))
        {
            for (const auto *v : cityPlayer.getVehicles())
            {
                ...
                {
                    cityPlayer.addReward(goalFirstEV + ", for purchasing your first EV.", 5000, 50);
    ...
    // display building types
    void showBuildingMenu()
    {
        cout << "\n--- Build Structure Menu ---" << endl;

        House tempHouse;
        cout << "1. House (" << displayCurrency(tempHouse.getCost()) << ")" << endl;
        SkylineResidence tempSky;
        cout << "2. Skyline Residence (" << displayCurrency(tempSky.getCost()) << ")" << endl;
        Mall tempMall;
        cout << "3. Mall (" << displayCurrency(tempMall.getCost()) << ")" << endl;
        PowerPlant tempCoal("Coal");
        cout << "4. Power Plant (Coal) (" << displayCurrency(tempCoal.getCost()) << ")" << endl;
        PowerPlant tempSolar("Solar");
        cout << "5. Power Plant (Solar) (" << displayCurrency(tempSolar.getCost()) << ")" << endl;
        PowerPlant tempWind("Wind");
        cout << "6. Power Plant (Wind) (" << displayCurrency(tempWind.getCost()) << ")" << endl;
        PowerPlant tempNuclear("Nuclear");
        cout << "7. Power Plant (Nuclear) (" << displayCurrency(tempNuclear.getCost()) << ")" << endl;
        cout << "0. Cancel" << endl;
        cout << "Enter choice: ";
        int choice;
        cin >> choice;
        ...
        case 1:
            ...
            break;
        case 2:
            building = new SkylineResidence();
            break;
        case 3:
            building = new Mall();
            break;
        case 4:
            building = new PowerPlant("Coal");
            break;
        case 5:
            building = new PowerPlant("Solar");
            break;
        case 6:
            building = new PowerPlant("Wind");
            break;
        case 7:
            building = new PowerPlant("Nuclear");
            break;
        case 0:
            cout << "Build cancelled." << endl;
            return;
        default:
    ...
    void showTransportMenu()
    {
        ...
        Road tempRoad;
        cout << "1. Road Network (" << displayCurrency(tempRoad.getCost()) << ")" << endl;
        Railways tempRail;
        cout << "2. Railway Network (" << displayCurrency(tempRail.getCost()) << ")" << endl;
        Airport tempAir;
        cout << "3. Airport (" << displayCurrency(tempAir.getCost()) << ")" << endl;
    ...
    void showVehicleMenu()
    {
        ...
        EV tempEV;
        cout << "1. Electric Vehicle (EV) (" << displayCurrency(tempEV.getCost()) << ") - EcoScore +" << tempEV.getEnvironmentalImpact() << endl;
        ICE tempICE;
        cout << "2. ICE Vehicle (" << displayCurrency(tempICE.getCost()) << ") - EcoScore " << tempICE.getEnvironmentalImpact() << endl;
        cout << "0. Cancel" << endl;
        cout << "Enter choice: ";
        int choice;
        cin >> choice;
    ...
    void DriveVehicle()
    {
        ...
        if (!size(myVehicles))
        {
            cout << "You currently don't own any vehicle.\n";
            return;
        }
        ...
        {
            cityPlayer.gainExperience(50);
            cityEnvironment.modifyPollution(2);
        }
        else
        {
            cityPlayer.gainExperience(15);
            cityEnvironment.modifyPollution(10);
        }
    }
    ...
    void BuyItemGeneric(itemType *newItem)
    {
        ...
        try
        {
            ...
            string identifier;
            double xpDivisor; // xpDivisor differs for each type, idenifier stores obj type

            if constexpr (is_same_v<itemType, Building>)
            {
                identifier = newItem->getBuildingType();
                xpDivisor = 150.0;
            }
            else if constexpr (is_same_v<itemType, Transport>)
            {
                identifier = "Transport Infrastructure";
                xpDivisor = 200.0;
            }
            else if constexpr (is_same_v<itemType, Vehicle>)
            {
                identifier = newItem->getType();
                xpDivisor = 250.0;
            }
...
void displayRankings(const vector<T *> &items, double (*getScore)(const T *) = [](const T *item)
                                               { return item->getEcoScore(); })
{
    cout << "\n--- Global Eco Rankings ---" << endl; // default Eco
    if (items.empty())
    {
        cout << "No items to rank." << endl;
        return;
    }
    ...
    mergeSort(ranking); // sort function from algorithms.
    ...
    cout << "-----+-------------------------+---------" << endl;
    for (int entry = 0; entry < ranking.size(); entry++) // iterate overall entries in ranking vector
...
int selectOpponentCity(const vector<City *> &cities, int currentCityIndex)
{
    ...
    if (cities.size() < 2)
    {
        cout << "Not enough cities to compete." << endl;
        return -1;
    } // city count need to be greater than 1
    int displayIndex = 1;
    ...
    {
        cout << "No opponents available." << endl;
        return -1;
    }
...
void simulation()
{
    ...
    { // masterChoice != 0
        ...
        case 1:
        {
            ...
            do
            {
                cout << "Enter city name (cannot be empty): ";
        ...
        case 2:
        {
            ...
            if (cities.empty())
            {
                cout << "No cities exist yet. Create one first." << endl;
                pause();
                break;
            }
        ...
        case 0: // Exit Simulation
            ...
            break;
        default:
            cout << "Invalid choice in master menu." << endl;
            pause();
            break;
        }
        ...
        if (activeCityIndex != -1 && masterChoice)
        {
            ...
            while (cityChoice != 42)
            {
                ...
                // Build Menu
                case 11:
                    currentCity->showBuildingMenu();
                    needsUpdate = true;
                    break; // Building changes state
                case 12:
                    currentCity->showTransportMenu();
                    needsUpdate = true;
                    break; // Transport changes state
                case 13:
                    currentCity->showVehicleMenu();
                    needsUpdate = true;
                    break; // Vehicles change state
                // Actions Menu
                case 21:
                    currentCity->GoToWork();
                    needsUpdate = true;
                    break; // Work changes state (pollution, gold)
                case 22:
                    currentCity->GoToMall();
                    break;
                case 23:
                    currentCity->DriveVehicle();
                    break;
                case 24:
                    currentCity->PlantTree();
                    needsUpdate = true;
                    break;
                // Management Menu
                case 31:
                    currentCity->PerformMaintenance();
                    needsUpdate = true;
                    break; // Maintenance changes state
                case 32:
                    cout << *currentCity;
                    break; // Showing details doesn't change state
                case 33:
                { // Compete
                    int opponentIndex = selectOpponentCity(cities, activeCityIndex);
                    if (opponentIndex != -1)
                        currentCity->Compete(*cities[opponentIndex]);
                    else
                        cout << "Competition cancelled or no opponent available." << endl;
                    break; // Competing doesn't change state directly
                }
                // Other Menu
                case 41:
                    displayRandomTip();
                    break; // Tip doesn't change state
                case 42:
                    cout << "Returning to Master Menu..." << endl;
                    break; // Exit city loop
                default:
                    cout << "Invalid choice for city action." << endl;
                    break;
                }
